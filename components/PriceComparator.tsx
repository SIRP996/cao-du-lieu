...
import { 
  ...
  TrendingUp, TrendingDown, Search, BarChart2, HelpCircle
} from 'lucide-react';
...
const PriceComparator: React.FC = () => {
  ...
  const renderSparkline = (row: ProcessedRow) => {
      ...
      if (data.length < 2) return <div className="text-xs text-slate-300 italic">--</div>;

      const isIncreasing = data[data.length - 1].price > data[0].price;
      const color = isIncreasing ? '#ef4444' : '#10b981';
            ...
            <div className="p-6 border-b border-slate-100 bg-gradient-to-r from-emerald-50 to-white">
                <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                    ...
                    <div className="flex items-center gap-3">
                        <input type="file" multiple accept=".xlsx, .xls" className="hidden" ref={fileInputRef} onChange={handleFileChange}/>
                        {!result ? (
                            <button onClick={() => fileInputRef.current?.click()} className="px-6 py-3 bg-slate-900 text-white font-bold rounded-xl text-xs uppercase tracking-widest hover:bg-slate-800 transition-all flex items-center gap-2 shadow-lg">
                                <Upload className="w-4 h-4" /> Tải Files
                            </button>
                        ) : (
                            <button onClick={handleReset} className="px-4 py-2 bg-slate-100 text-slate-500 font-bold rounded-xl text-xs uppercase tracking-widest hover:bg-slate-200 transition-all flex items-center gap-2">
                                <Trash2 className="w-4 h-4" /> Reset
                            </button>
                        )}
                        {files.length > 0 && !result && (
                            <button onClick={handleProcess} disabled={status.step === 'processing'} className="px-6 py-3 bg-emerald-600 text-white font-bold rounded-xl text-xs uppercase tracking-widest hover:bg-emerald-700 transition-all flex items-center gap-2 shadow-lg shadow-emerald-200">
        ...
        {result && (
            <div className="bg-white rounded-2xl shadow-lg border border-slate-100 flex flex-col min-h-[400px]">
                ...
                <div className="p-4 border-b border-slate-100 flex justify-between items-center sticky top-0 bg-white z-20 rounded-t-2xl">
                    ...
                    <button onClick={handleDownload} className="flex items-center gap-2 px-4 py-2 bg-emerald-50 text-emerald-700 font-bold rounded-lg text-xs uppercase hover:bg-emerald-100 transition-all">
                ...
                {rows.length === 0 ? (
                    <div className="flex-1 flex flex-col items-center justify-center py-20 text-center px-4">
                        ...
                        <p className="text-slate-500 text-sm max-w-md mb-6">
                            Đã đọc file nhưng không tìm thấy cột dữ liệu hợp lệ.
                        </p>
                        <div className="bg-slate-50 border border-slate-100 rounded-xl p-4 text-left text-sm text-slate-600 max-w-md w-full">
                            <div className="flex items-center gap-2 mb-2 font-bold text-indigo-600">
                                <HelpCircle className="w-4 h-4"/> Gợi ý sửa file Excel:
                            </div>
                            <ul className="list-disc list-inside space-y-1 text-xs">
                                <li>Đảm bảo dòng tiêu đề (Header) chứa từ: <strong>"Tên sản phẩm", "Product Name", "Tên"</strong>.</li>
                                <li>Đảm bảo cột giá chứa từ: <strong>"Giá", "Price", "Đơn giá"</strong>.</li>
                                <li>File không nên có quá nhiều dòng trống ở đầu (Hệ thống sẽ tự quét 10 dòng đầu).</li>
                            </ul>
                        </div>
                                ...
                                {processedRows.map((row, idx) => (
                                    <tr key={idx} className="hover:bg-slate-50/80 transition-colors group">
                                        ...
                                        {result.columns.map((col, cIdx) => {
                                            ...
                                            return (
                                                <td key={col} className={`p-4 text-right align-top ${isCheaper ? 'bg-emerald-50/30' : ''}`}>
                                                    <div className={`font-mono font-bold ${isCheaper ? 'text-emerald-600' : 'text-slate-600'}`}>
                                                        {price > 0 ? price.toLocaleString('vi-VN') : '-'}
                                                    </div>
                                                    {diffPercent !== 0 && (
                                                        <div className={`text-[10px] font-bold flex items-center justify-end gap-0.5 mt-1 ${diffPercent < 0 ? 'text-emerald-500' : 'text-rose-500'}`}>
                                                            {diffPercent < 0 ? <TrendingDown className="w-3 h-3"/> : <TrendingUp className="w-3 h-3"/>}
